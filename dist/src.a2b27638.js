...
function initializeCode() {
  var form = document.getElementById("user"); //The form logic partially taken from https://www.javascripttutorial.net/javascript-dom/javascript-form/

  form.addEventListener("submit", function (event) {
    // stop form submission
    event.preventDefault();
    delExisting(form);
    var dbBody = document.getElementById("database-body");
    var tr = document.createElement("tr");

    for (var x = 1; x < 4; x++) {
      var _td = document.createElement("td");

      _td.appendChild(document.createTextNode(form.elements[x].value));
...
function delExisting(form) {
  var rows = document.getElementById("database-body").rows;
